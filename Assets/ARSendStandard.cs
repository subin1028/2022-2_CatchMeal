...
public static class ShareValue{
     public static float sharedvalue;
}

public class ARSendStandard : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject.Find("textButton").GetComponentInChildren<Text>().fontSize = 30;
        GameObject.Find("textButton").GetComponentInChildren<Text>().alignment = TextAnchor.MiddleCenter;
        openJaw = false;
        count = 0;
        check = false;
        set = false;

        cngbutton.onClick.AddListener(changeset);
        cngbutton.onClick.AddListener(cngScene);

        /*DatabaseReference reference = FirebaseDatabase.DefaultInstance.RootReference;
	    reference.Child("EatingInterest").GetValueAsync().ContinueWith( task => {
            		if(task.IsCompleted){
                		DataSnapshot snapshot = task.Result;
                		Debug.Log(snapshot.Child("Ndistance").Value.ToString());
           		}
        	});	*/
    }
    ...
    void Update()
    {
        ...
        foreach (var face in mArFaceManager.trackables)        
        {
            ...
            // void SetOnClick(){
            //         GameObject.Find("set").GetComponentInChildren<Text>().text = "\t" + distance;
            // }
            if(distance > 0){
                makeLog("준비를 마쳤다면 버튼 클릭!");
            }
            ...
            if (set == true){
                ...
                .Child("Ndistance")
                .SetValueAsync(distance);
                ShareValue.sharedvalue = distance;
                set = false;
            }
        }
    }
    ...
    void makeLog(string str)
    {
        ...
        GameObject.Find("textButton").GetComponentInChildren<Text>().text = "\t" + str;
    }

    void makeLogXYZ(string x, string y, string z)
    {
        GameObject.Find("textButton").GetComponentInChildren<Text>().text
            = "\tX: " + x + "\r\n"
            + "\tY: " + y + "\r\n"
            + "\tZ: " + z;
    }
